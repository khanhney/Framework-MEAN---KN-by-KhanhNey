...
// get search Product
route.post('/searchProduct', bodyParser, (req, res)=>{
    const  searchProduct  = req.body.searchProduct;
    // console.log({ searchProduct });
    Product.find({ $text: {$search: searchProduct}})
    .then(result => {
        Anhchinh.find({}).then(result2 =>{
            res.render('index', { data: result, data2: result2 });
        })
    })
});
// route post sua
route.post('/suaProduct/:id', upload.single('imageProduct'), (req, res)=>{
    ...
    Product.findByIdAndUpdate(id, {imageProduct, nameProduct, priceProduct, newsProduct})
    .then(()=>{
